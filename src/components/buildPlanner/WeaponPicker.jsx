...
export default function WeaponPicker({ data, slotIndex, onClose, onSelectTalent }) {
  const { dispatch, canEquipExoticWeapon } = useBuild()
  const [search, setSearch] = useState('')
  const [typeFilter, setTypeFilter] = useState('all')

  const allWeapons = useMemo(() => data.armes || [], [data])
  ...
  const select = (weapon) => {
    dispatch({ type: 'SET_WEAPON', slot: slotIndex, weapon })
    onSelectTalent(slotIndex)
  }

  const canEquipExotic = canEquipExoticWeapon(slotIndex)
  ...
  return (
    <SelectionModal
      open={true}
      title={`Sélection — ${['Arme Primaire', 'Arme Secondaire', 'Arme de Poing'][slotIndex]}`}
      onClose={onClose}
      searchValue={search}
      onSearch={setSearch}
